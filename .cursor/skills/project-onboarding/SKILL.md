...
description: Use when starting a new project from this template, analyzing the brief, defining project scope and size, preparing the TECH_CARD, and establishing the initial architecture and delivery workflow.
---
...
6. **Design architecture** — choose the folder layout for the confirmed size, follow the stack in TECH_CARD, apply [the architecture rule](../../rules/01-architecture.mdc), and document the result in `docs/01-ARCHITECTURE.md`.
7. **Initialize the project** — scaffold the agreed application with `pnpm`, create `.env.example` without secrets, and establish the minimal documentation set.
8. **Configure the quality baseline** — set up the agreed checks and hooks according to [CI/CD rules](../../rules/17-cicd.mdc) and the [quality CI example](../../../docs/reference/workflows/ci-quality.yml.example).
...
10. **Prepare release** — perform the final TECH_CARD and quality review, configure deployment environment variables, apply migrations safely, and verify monitoring according to [ops and reliability rules](../../rules/14-ops.mdc).
...
- Permanent engineering standards remain in `../../rules/` and apply throughout the workflow.
